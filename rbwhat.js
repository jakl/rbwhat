...
(function() {
  ...
  formatBug = function(bug) {
    return "  " + (pad(config.bugUrl.grey + bug.white, 24));
  };

  formatGit = function(branch, repo) {
    var branchColored, repoColored, urlColored;
    urlColored = config.gitUrl.grey;
    branchColored = (branch || 'No Branch').white;
    repoColored = (repo || 'No Repo').white;
    if (branch && repo) {
      return "  " + urlColored + repoColored + config.branchWedge.grey + branchColored;
    } else {
      return "  " + urlColored + repoColored + "  " + branchColored;
    }
  };

  formatUrl = function(request_id) {
    var diff;
